...
@section('title', trans('applang.roles'))
        ...
        <div class="default-app-list-table">
            <div class="card">
                <div class="card-header justify-content-start">
                    <a href="{{route('roles.create')}}" class="btn btn-success mb-0">
                        ...
                        <table id="default-app-datatable" class="stripe hover bordered datatable datatable-user" style="width: 100%">
                            <thead>
                                <tr>
                                    ...
                                    <th class="pl-10 no-wrap">{{trans('applang.users')}}</th>
                                    <th class="pl-10 no-wrap">{{trans('applang.permissions')}}</th>
                            ...
                            <tbody>
                              @if ($roles->count())
                                    @foreach ($roles as $role)
                                        <tr id="selected_row_{{$role->role_id}}">
                                            ...
                                            <td class="middle-align normal-wrap">{{$role->role_id}}</td>
                                            <td class="middle-align normal-wrap" style="width: 10%">
                                                {{app()->getLocale() == 'ar' ? $role->name_ar : $role->name_en}}
                                            </td>
                                            <td class="middle-align normal-wrap">
                                                <a href="{{route('usersOfRoleQuery', $role->role_id)}}">
                                                    {{usersOfRole($role->name_en)}} {{trans('applang.user')}}
                                                </a>
                                            </td>
                                            <td class="middle-align normal-wrap w-75">
                                                    {!! "<span class='badge badge-light-secondary mr-1 badge-lower'>"
                                                    .implode('</span><span class="badge badge-light-secondary mr-1 badge-lower">', permissionsOfRoleName($role->role_id))
                                                    ."</span>"!!}
                                            </td>
                                            <td class="middle-align normal-wrap" style="width: 10%">
                                                <a  href="{{route('roles.edit', $role)}}" title="{{trans('applang.edit')}}">
                                                ...
                                                <a href="#"
                                                    ...
                                                    data-toggle="modal"
                                                    data-target="#formModalDeleteRole"
                                                    data-role_id="{{$role->role_id}}"
                                                    data-name_ar="{{$role->name_ar}}"
                                                    data-name_en="{{$role->name_en}}"
                                                    >
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#formModalDeleteRole').on('show.bs.modal', function (event) {
