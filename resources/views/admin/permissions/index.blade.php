...
@section('title', trans('applang.permissions'))
        ...
        <div class="default-app-list-table">
            <div class="card">
                <div class="card-header justify-content-start">
                    <a href="{{route('permissions.create')}}" class="btn btn-success mb-0">
                        ...
                        <table id="default-app-datatable" class="stripe hover bordered datatable datatable-user" style="width: 100%">
                            <thead>
                                <tr>
                                    ...
                                    <th class="pl-10 no-wrap">{{trans('applang.departments')}}</th>
                            ...
                            <tbody>
                              @if ($permissions->count())
                                    @foreach ($permissions as $permission)
                                        <tr id="selected_row_{{$permission->id}}">
                                            <td class="select_all normal-wrap">
                                                <fieldset>
                                                    {{-- <div class="checkbox checkbox-sm checkbox-glow checkbox-danger"> --}}
                                                        ...
                                                        <label for="{{$permission->permission_id}}"></label>
                                                    {{-- </div> --}}
                                                </fieldset>
                                            </td>
                                            <td class="normal-wrap">{{$permission->permission_id}}</td>
                                            <td class="normal-wrap w-25">
                                                {{app()->getLocale() == 'ar' ? $permission->name_ar : $permission->name_en}}
                                            </td>
                                            <td class="normal-wrap w-25">
                                                @if ($permission->category)
                                                    {{app()->getLocale() == 'ar' ? $permission->category->name_ar : $permission->category->name_en}}
                                                @endif
                                            </td>
                                            <td class="normal-wrap">
                                                <a  href="{{route('permissions.edit', $permission->permission_id)}}" title="{{trans('applang.edit')}}">
                                                ...
                                                <a href="#"
                                                    ...
                                                    data-toggle="modal"
                                                    data-target="#formModalDeletePermission"
                                                    data-permission_id="{{$permission->id}}"
                                                    data-name_ar="{{$permission->name_ar}}"
                                                    data-name_en="{{$permission->name_en}}"
                                                    >
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#formModalDeletePermission').on('show.bs.modal', function (event) {
        ...
        $('#deleteAllSelectedRecords').click(function(e){
            ...
            Swal.fire({
                ...
                if(result.isConfirmed) {
                    $.ajax({
                        url:"{{route('deleteSelectedPermissions')}}",
                        type:"DELETE",
                        data:{
