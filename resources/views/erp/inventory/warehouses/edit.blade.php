...
@section('title', trans('applang.edit_warehouse'))
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header modal-header bg-primary">
                        <h4 class="modal-title white">{{trans('applang.edit_warehouse')}}</h4>
                    </div>
                    <div class="card-body mt-1" style="padding-bottom: 13px">
                        <form action="{{route('warehouses.update', $warehouse)}}" method="POST">
                                ...
                                <fieldset class="form-group">
                                    <select id="branch_id" class="custom-select @error('branch_id') is-invalid @enderror" name='branch_id'>
                                        ...
                                        @foreach($branches as $branch)
                                            <option value="{{  $branch->id }}" {{$warehouse->branch->id == $branch->id ? 'selected' : ''}}>
                            ...
                            <div class="form-group col-md-12 mb-50">
                                ...
                                >{{$warehouse->shipping_address ?? old('shipping_address')}}</textarea>
                            ...
                            <div class="form-group col-md-12 mb-50">
                                ...
                                <fieldset class="form-group mb-50">
                                    <select id="status" class="custom-select @error('status') is-invalid @enderror" name='status'>
                                        ...
                                        <option value="0" {{$warehouse->status == 0 ? 'selected' : ''}}>{{trans('applang.suspended')}}</option>
                                        <option value="1" {{$warehouse->status == 1 ? 'selected' : ''}}>{{trans('applang.active')}}</option>
                                    </select>
