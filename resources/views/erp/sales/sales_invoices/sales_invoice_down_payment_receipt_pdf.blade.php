...
<div class="receipt">
    <table cellpadding="0" cellspacing="0" width="100%" class="receipt-header">
        <tr>
            ...
            <td align="start" width="30%" style="vertical-align: top !important">
                <span ><b>{{trans('applang.the_date')}}:</b> {{$salesInvoice->issue_date}}</span>
            </td>
            <td align="start" width="10%" style="vertical-align: top !important">
                <span ><b>{{trans('applang.number')}}:</b> {{$salesInvoice->number}}</span>
            </td>
        </tr>
    </table>
    <hr>
    <table cellpadding="0" cellspacing="0" width="100%" class="receipt-body">
        <tr>
            <td>
                <span><b>{{trans('applang.from')}}: </b> {{$salesInvoice->client->full_name}}</span>
            </td>
        </tr>
        <tr>
            <td width="25%">
            ...
            <td width="75%">
                <span ><b>{{trans('applang.paid_by')}}: </b> {{$salesInvoice->deposit_payment_method}}</span>
            </td>
        </tr>
        <tr>
            <td colspan="5">
                <span ><b>{{trans('applang.sales_invoice_number')}}: </b>#{{$salesInvoice->inv_number}}</span>
            </td>
        </tr>
        <tr>
            <td>
                <span ><b>{{trans('applang.received_by')}}: </b> {{$salesInvoice->user->first_name}} {{$salesInvoice->user->last_name}}</span>
            </td>
        </tr>
        <tr>
            <td>
                @if($salesInvoice->notes != null)
                    <span ><b>{{trans('applang.purpose')}}: </b> {!!$salesInvoice->notes!!}</span>
                @endif
            </td>
        </tr>
    </table>
