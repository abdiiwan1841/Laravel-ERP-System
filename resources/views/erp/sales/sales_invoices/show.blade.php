...
@section('title', trans('applang.show_sales_invoice'))
    ...
    <div class="container-fluid">
        <div class="content-body">
            <div class="row justify-content-center">
                <div class="col-md-10">
                    @livewire('erp.sales.sales-invoices.show-sales-invoice', ['salesInvoice' => $salesInvoice])
                </div>
            </div>
        </div>
    </div>
    <!-- sales invoices Modals -->
    ...
    <script>
        $(document).ready(function () {
            ...
            showDownPaymentModal.on('show.bs.modal', function (event) {
                ...
                var salesInvoice_id = button.data('sales_invoice_id');
                $.get("{{\Illuminate\Support\Facades\URL::to('/'.app()->getLocale().'/erp/sales/sales-invoice-show-down-payment-ajax')}}/"+salesInvoice_id , function (data) {
                    console.log(data);
                    ...
                    if(data.salesInvoice.deposit_payment_method === 'cash'){
                        down_deposit_payment_method.text('{{trans('applang.cash')}}');
                    }else if(data.salesInvoice.deposit_payment_method === 'cheque'){
                        down_deposit_payment_method.text('{{trans('applang.cheque')}}');
                    }else if(data.salesInvoice.deposit_payment_method === 'bank_transfer'){
                    ...
                    down_transaction_id.text(data.salesInvoice.deposit_transaction_id);
                    down_pmt_status.text('{{trans("applang.down_payment")}}');
                    down_payment_date.text(data.salesInvoice.issue_date);
            ...
            showPaymentModal.on('show.bs.modal', function (event) {
                ...
                $.get("{{\Illuminate\Support\Facades\URL::to('/'.app()->getLocale().'/erp/sales/sales-invoice-show-payment-transaction-ajax')}}/"+payment_id , function (data) {
                    // console.log(data);
            ...
            $('.edit-payment').on('click', function (e) {
                ...
                $.get("{{\Illuminate\Support\Facades\URL::to('/'.app()->getLocale().'/erp/sales/sales-invoice-edit-payment-transaction-ajax')}}/"+pmt_id , function (data) {
                    console.log(data)
            ...
            $('.print-receipt-ajax').on('click', function (e) {
                e.preventDefault()
                var pmt_id =$('#payment_id').text();
                $.get("{{\Illuminate\Support\Facades\URL::to('/'.app()->getLocale().'/erp/sales/sales-payment-receipt-print-ajax')}}/"+pmt_id , function (data) {
                    // console.log(data)
            ...
            $('#check').on('click', function (e) {
                e.preventDefault();
                var pmt_id =$('#payment_id').text();
                $.get("{{\Illuminate\Support\Facades\URL::to('/'.app()->getLocale().'/erp/sales/sales-invoice-complete-payment-transaction')}}/"+pmt_id , function (data) {
                    console.log(data)
                    const Toast = Swal.mixin({
                        toast: true,
                        position: document.dir === 'rtl' ? "top-start" : "top-end",
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#formModalEditPaymentDirect').on('show.bs.modal', function (event) {
                ...
                $.get("{{\Illuminate\Support\Facades\URL::to('/'.app()->getLocale().'/erp/sales/sales-invoice-edit-payment-transaction-ajax')}}/"+payment_id , function (data) {
                    console.log(data)
                    $('#payment_id-edit').text(data.paymentEdit.id);
