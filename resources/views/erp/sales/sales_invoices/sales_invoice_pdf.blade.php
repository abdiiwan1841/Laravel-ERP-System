...
<head>
    <meta charset="utf-8" />
    <title>{{trans('applang.sales_invoice')}}: # {{$salesInvoice->inv_number}}</title>
...
<div class="invoice-box {{app()->getLocale() == 'ar' ? 'rtl' : ''}}">
    <table cellpadding="0" cellspacing="0">
        <tr class="top">
            <td colspan="6">
                <table class="inv-header">
                    <tr>
                        <td class="inv-type">
                            <h3>{{trans('applang.sales_invoice')}}</h3>
                ...
                <table>
                    <tr>
                        <td width="60%">
                            ...
                            <span>{{$salesInvoice->client->full_name}}</span><br/>
                            @if($salesInvoice->client->street_address != null)
                                <span>{{$salesInvoice->client->street_address}}</span><br/>
                                <span>{{$salesInvoice->client->state}}, {{$salesInvoice->client->city}}, {{$salesInvoice->client->postal_code}}</span><br/>
                            @endif
                        </td>

                        <td align="start" width="40%">
                            <span><b>{{trans('applang.invoice_number')}}# </b>{{$salesInvoice->inv_number}}</span><br/>
                            <span><b>{{trans('applang.issue_date')}}: </b>{{$salesInvoice->issue_date}}</span><br/>
                            <span><b>{{trans('applang.due_date')}}: </b>{{$salesInvoice->due_date}}</span><br/>
        ...
        <tr class="total">
            ...
            <td class="total-el {{app()->getLocale() == 'ar' ? 'align-l' : 'align-r'}}">{{$salesInvoice->subtotal}}</td>
        </tr>
        @if($salesInvoice->discount_inv != null)
            <tr class="total">
                <td colspan="3"></td>
                <td colspan="2" class="total-el">{{trans('applang.discount')}}</td>
                <td class="total-el {{app()->getLocale() == 'ar' ? 'align-l' : 'align-r'}}">{{$salesInvoice->discount_inv}}</td>
            </tr>
        @endif
        ...
        @if($salesInvoice->shipping_expense_inv != null)
            <tr class="total">
                ...
                <td class="total-el {{app()->getLocale() == 'ar' ? 'align-l' : 'align-r'}}">{{$salesInvoice->shipping_expense_inv}}</td>
            </tr>
        @endif
        @if($salesInvoice->down_payment_inv != null)
            <tr class="total">
                ...
                <td class="total-el {{app()->getLocale() == 'ar' ? 'align-l' : 'align-r'}}">{{$salesInvoice->down_payment_inv}}</td>
            </tr>
        @endif
        <tr class="total">
            <td colspan="3"></td>
            <td colspan="2" class="total-el due">{{trans('applang.due_amount')}}</td>
            <td class="total-el due {{app()->getLocale() == 'ar' ? 'align-l' : 'align-r'}}">{{$salesInvoice->due_amount}}</td>
        </tr>
        @if($salesInvoice->paid_to_supplier_inv != null)
            <tr class="total">
                <td colspan="3"></td>
                <td colspan="2" class="total-el">{{trans('applang.paid')}}</td>
                <td class="total-el {{app()->getLocale() == 'ar' ? 'align-l' : 'align-r'}}">{{$salesInvoice->paid_to_supplier_inv}}</td>
            </tr>
            <tr class="total">
                <td colspan="3"></td>
                <td colspan="2" class="total-el due">{{trans('applang.due_amount_after_paid')}}</td>
                <td class="total-el due {{app()->getLocale() == 'ar' ? 'align-l' : 'align-r'}}">{{$salesInvoice->due_amount_after_paid}}</td>
            </tr>
        @endif
        @if($salesInvoice->payments_total > 0.00)
            <tr class="total">
                ...
                <td class="total-el {{app()->getLocale() == 'ar' ? 'align-l' : 'align-r'}}">-{{number_format($salesInvoice->payments_total,0,'.','')}} {{$companyData->basic_currency_symbol}}</td>
            </tr>
            <tr class="total">
                <td colspan="3"></td>
                <td colspan="2" class="total-el due">{{trans('applang.due_amount_after_payments')}}</td>
                <td class="total-el due {{app()->getLocale() == 'ar' ? 'align-l' : 'align-r'}}">{{number_format($salesInvoice->due_amount_after_payments, 0, '.', '')}} {{$companyData->basic_currency_symbol}}</td>
            </tr>
        @endif
    ...
    <!-- invoice notes -->
        @if($salesInvoice->notes != '')
            ...
            <tr class="inv-notes">
                <td colspan="6" class="text">{!! $salesInvoice->notes !!}</td>
            </tr>
        @endif
    </table>
</div>
