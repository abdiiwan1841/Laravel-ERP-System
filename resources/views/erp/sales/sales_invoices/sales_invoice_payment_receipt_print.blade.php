...
<head>
    ...
    <title>{{trans(trans('applang.payment_transaction'))}} # {{$payment->id}} {{trans('applang.sales_invoice')}} # {{$payment->salesInvoice->inv_number}}</title>
            ...
            <div class="card invoice-print-area mt-5 mb-5 black">
                <div class="card-body pb-0 mx-25">
                    <div class="receipt mb-5 p-1 ">
                        <div class="receipt-header d-flex justify-content-between">
                            <div class="mb-50">
                                <h3 class="text-bold-700 black">{{trans('applang.payment_receipt')}}</h3>
                            ...
                            <div>
                                <span class="font-small mr-5 ml-5"><b>{{trans('applang.the_date')}}:</b> {{$payment->payment_date}}</span>
                                <span class="font-small"><b>{{trans('applang.number')}}:</b> {{$payment->id}}</span>
                            </div>
                        </div>

                        <div class="receipt-body">
                            <span class="font-small d-block"><b>{{trans('applang.from')}}: </b> {{$payment->salesInvoice->client->full_name}}</span>
                            <span class="d-block">
                                    <span class="font-small "><b>{{trans('applang.amount')}}: </b> {{$payment->payment_amount}} {{$currency_symbol}}</span>
                                    <span class="font-small ml-2 mr-2"><b>{{trans('applang.paid_by')}}: </b> {{$payment->deposit_payment_method}}</span>
                                </span>
                            <span class="font-small d-block"><b>{{trans('applang.sales_invoice_number')}}: </b>#{{$payment->salesInvoice->inv_number}}</span>
                            <span class="font-small d-block"><b>{{trans('applang.received_by')}}: </b> {{$payment->employee->first_name}} {{$payment->employee->last_name}}</span>
                            @if($payment->receipt_notes != null)
                                <span class="font-small d-block"><b>{{trans('applang.purpose')}}: </b> {{$payment->receipt_notes}}</span>
                            @endif
                        </div>
                ...
                <div class="card-footer d-flex d-print-none {{app()->getLocale() == 'ar' ? 'justify-content-end' : 'justify-content-start'}}" style="font-family: 'Nunito', sans-serif !important">
                    @if(app()->getLocale() == 'ar')
                        ...
                        <a href="{{route('SalesInvoicePaymentReceiptPdf', $payment->id)}}" target="_blank" class="btn btn-instagram ml-50 mr-50"><i class="bx bxs-file-pdf"></i> PDF</a>
