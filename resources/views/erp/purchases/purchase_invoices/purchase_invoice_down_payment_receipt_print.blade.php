            ...
            <div class="card invoice-print-area mt-5 mb-5 black">
                <div class="card-body pb-0 mx-25">
                    <div class="receipt mb-5 p-1 ">
                        <div class="receipt-header d-flex justify-content-between">
                            ...
                            <div>
                                <span class="font-small mr-5 ml-5"><b>{{trans('applang.the_date')}}:</b> {{$purchaseInvoice->issue_date}}</span>
                                <span class="font-small"><b>{{trans('applang.number')}}:</b> {{$purchaseInvoice->number}}</span>
                            </div>
                        </div>

                        <div class="receipt-body">
                            <span class="font-small d-block"><b>{{trans('applang.to')}}: </b> {{$purchaseInvoice->supplier->first_name}} {{$purchaseInvoice->supplier->last_name}}</span>
                            <span class="d-block">
                                    <span class="font-small "><b>{{trans('applang.amount')}}: </b> {{$down_payment}} {{$currency_symbol}}</span>
                                    <span class="font-small ml-2 mr-2"><b>{{trans('applang.paid_by')}}: </b> {{$purchaseInvoice->deposit_payment_method}}</span>
                                </span>
                            <span class="font-small d-block"><b>{{trans('applang.purchase_invoice_number')}}: </b>#{{$purchaseInvoice->inv_number}}</span>
                            <span class="font-small d-block"><b>{{trans('applang.received_by')}}: </b> {{$purchaseInvoice->user->first_name}} {{$purchaseInvoice->user->last_name}}</span>
                            @if($purchaseInvoice->notes != null)
                                <span class="font-small d-block"><b>{{trans('applang.notes')}}: </b> {!!$purchaseInvoice->notes!!}</span>
                            @endif
                        </div>
                ...
                <div class="card-footer d-flex d-print-none {{app()->getLocale() == 'ar' ? 'justify-content-end' : 'justify-content-start'}}" style="font-family: 'Nunito', sans-serif !important">
                    @if(app()->getLocale() == 'ar')
                        ...
                        <a href="{{route('downPaymentReceiptPdf', $purchaseInvoice->id)}}" target="_blank" class="btn btn-instagram ml-50 mr-50"><i class="bx bxs-file-pdf"></i> PDF</a>
