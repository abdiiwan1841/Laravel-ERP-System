@if (\App\Models\ERP\Settings\Tax::count() > 0)
    <!--Start Delete Modal -->
    <div class="modal fade text-left" id="formModalDeleteTax" tabindex="-1" role="dialog" aria-labelledby="myModalLabel17" aria-hidden="true">
                    ...
                    <form action="{{route('taxes.destroy', 'test')}}" method="POST">
                        ...
                        @if (app()->getLocale() == 'ar')
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="tax_name_ar" id="tax_name_ar" readonly>
                            </div>
                        @else
                            <div class="form-group">
                                <p class="text-danger font-weight-bold">{{trans('applang.confirm_delete_msg')}}</p>
                                <input type="text" class="form-control" name="tax_name_en" id="tax_name_en" readonly>
                            </div>
                        @endif
