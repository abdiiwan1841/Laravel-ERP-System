<!--Start ِAdd SubCategories Modal -->
<div class="modal fade text-left closeModal" id="formModalAddSubCategory" tabindex="-1" role="dialog" aria-labelledby="myModalLabel17" aria-hidden="true" wire:ignore.self>
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h4 class="modal-title white" id="myModalLabel17">{{trans('applang.add_sub_category')}}</h4>
                ...
                <form wire:submit.prevent="store" method="POST" enctype="multipart/form-data" autocomplete="off">
                    ...
                    @forelse($addMore as $more)
                        <div class="form-row">
                            ...
                            <div class="col-sm-4">
                                <div class="form-group mb-50">
                                    <label for="category_id.{{$more}}" class="required">{{trans('applang.the_category')}}</label>
                                    <select id="category_id.{{$more}}" class="custome-select form-control @error('category_id') is-invalid @enderror" wire:model="category_id.{{$more}}">
...
<!--End Add SubCategories Modal -->

<!--Start Edit SubCategories Modal -->
<div class="modal fade text-left closeModal forceClose" id="formModalEditSubCategory" tabindex="-1" role="dialog" aria-labelledby="myModalLabel17" aria-hidden="true" wire:ignore.self>
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h4 class="modal-title white" id="myModalLabel17">{{trans('applang.edit_sub_category')}}</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="bx bx-x"></i>
                </button>
            </div>
            <div class="modal-body">
                <form wire:submit.prevent="update" method="POST" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="form-row">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group mb-50">
                                <label for="category_id" class="required">{{trans('applang.the_category')}}</label>
                                <select id="category_id}" class="custome-select form-control @error('category_id') is-invalid @enderror" wire:model="category_id">
                                    <option value="#" selected>{{trans('applang.select_the_category')}}</option>
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}">{{$category->name}}</option>
                                    @endforeach
