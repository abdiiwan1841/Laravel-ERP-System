<div class="card">
    <div class="card-header modal-header bg-primary">
        <h4 class="modal-title white">{{trans('applang.edit_purchase_invoice')}} # ({{$purchaseInvoice->inv_number}})</h4>
    </div>
    ...
    <div class="card-body mt-1" style="padding-bottom: 13px">
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <fieldset class="form-group">
                    <select id="supplier_id" class="custom-select @error('supplier_id') is-invalid @enderror" name='supplier_id' wire:model="supplier_id">
                ...
                @if($supplier_id == $purchaseInvoice->supplier_id)
                    <div class="supplier_invoice">
                        <h4 class="font-weight-bold black">{{$purchaseInvoice->supplier->commercial_name}}</h4>
                        <div class="d-flex mt-50">
                            <span class="font-weight-bold text-dark" style="width: 35%">{{trans('applang.commercial_record')}}:</span>
                            <span style="width: 65%">{{$purchaseInvoice->supplier->commercial_record}}</span>
                        </div>
                        <div class="d-flex mt-50">
                            <span class="font-weight-bold text-dark" style="width: 35%">{{trans('applang.tax_registration')}}:</span>
                            <span style="width: 65%">{{$purchaseInvoice->supplier->tax_registration}}</span>
                        </div>
                        <div class="d-flex mt-50">
                            <span class="font-weight-bold text-dark" style="width: 35%">{{trans('applang.name')}}:</span>
                            <span style="width: 65%">
                                {{$purchaseInvoice->supplier->first_name}}
                                {{$purchaseInvoice->supplier->last_name}}
                            </span>
                        </div>
                        <div class="d-flex mt-50">
                            <span class="font-weight-bold text-dark" style="width: 35%">{{trans('applang.address')}}:</span>
                            <div style="width: 65%">
                                <span style="display: block">{{$purchaseInvoice->supplier->street_address}}</span>
                ...
                @elseif($supp)
                    <div class="supplier_invoice">
                        <h4 class="font-weight-bold black">{{$supp->commercial_name}}</h4>
                        <div class="d-flex mt-50">
                            <span class="font-weight-bold text-dark" style="width: 35%">{{trans('applang.commercial_record')}}:</span>
                            <span style="width: 65%">{{$supp->commercial_record}}</span>
                        </div>
                        <div class="d-flex mt-50">
                            <span class="font-weight-bold text-dark" style="width: 35%">{{trans('applang.tax_registration')}}:</span>
                            <span style="width: 65%">{{$supp->tax_registration}}</span>
                        </div>
                        <div class="d-flex mt-50">
                            <span class="font-weight-bold text-dark" style="width: 35%">{{trans('applang.name')}}:</span>
                            <span style="width: 65%">{{$supp->first_name}} {{$supp->last_name}}</span>
                        </div>
                        <div class="d-flex mt-50">
                            <span class="font-weight-bold text-dark" style="width: 35%">{{trans('applang.address')}}:</span>
                            <div style="width: 65%">
                                <span style="display: block">{{$supp->street_address}}</span>
                                <span style="display: block">{{$supp->city}}, {{$supp->state}}, {{$supp->country}}</span>
                            </div>
                        </div>
                    </div>
                @else
            ...
            <div class="col-md-6">
                ...
                <div class="position-relative has-icon-left mb-50">
                    <input id="inv_number"
                           ...
                           value="{{$purchaseInvoice->inv_number}}"
                           readonly>
                ...
                <div class="position-relative has-icon-left mb-50">
                    <input type="text"
                           ...
                           name="issue_date"
                           value="{{$purchaseInvoice->issue_date}}"
                           readonly
                    >
                    <div class="form-control-position">
                ...
                <div class="position-relative has-icon-left">
                    <input type="text"
                           ...
                           name="due_date"
                           value="{{$purchaseInvoice->due_date ?? date('Y-m-d')}}"
                    >
                    <div class="form-control-position">
                        <i class="bx bx-calendar"></i>
                    </div>
        ...
        <div class="table-responsive">
            <table class="table table-small-font table-bordered inv-table" id="tax_table" style="width: 100%">
                ...
                <tbody>
                    ...
                    <!--down payment-->
                    @if($down_payment && $down_payment_inv && $down_payment_type >= 0 && $subtotal != '' && $deposit_is_paid == true)
                        <tr style="background-color: #EEEEEE; font-weight: bolder">
                            <td class="table-invoice-totals" style="border-bottom: none" colspan="{{count($taxes) > 1 ? '5' : '4'}}"></td>
                                ...
                                <fieldset class="form-group">
                                    <select id="warehouse_id" class="custom-select @error('warehouse_id') is-invalid @enderror" name="warehouse_id">
                                        ...
                                        @foreach($warehouses as $key => $warehouse)
                                            <option value="{{$warehouse->id}}" {{$purchaseInvoice->warehouse_id == $warehouse->id ? 'selected' : ''}}>{{$warehouse->name}}</option>
                                        @endforeach
                                    </select>
        ...
        <div class="mt-1 mb-50" wire:ignore>
            ...
            <textarea id="editor" name="notes">{{$purchaseInvoice->notes}}</textarea>
        </div>
