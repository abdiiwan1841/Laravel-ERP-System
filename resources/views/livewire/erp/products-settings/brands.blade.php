<div class="content-body">
    <div class="card">
        ...
        <div class="card-header justify-content-between bg-secondary bg-lighten-4" style="border-bottom: 1px solid #DDD">
            <div class="btn-group justify-content-start align-items-center">
                <a href="#" data-toggle="modal" data-target="#formModalAddBrand" class="btn btn-success mb-0">
        ...
        <div class="card-body pt-1">
                    ...
                    <div class="col-md-12 mb-2">
                        <div class="d-flex justify-content-start align-items-center">
                            @if($selectAll)
                                <div class="d-flex justify-content-start align-items-center">
                                    <p class="mb-0">
                                        {{trans('applang.you_have_selected_all')}} ( <strong>{{$brands->total()}}</strong> {{trans('applang.items')}} ).
                                    </p>
                                </div>
                            @else
                                <div class="d-flex justify-content-start align-items-center">
                                    <p class="mb-0">
                                        ...
                                        {{trans('applang.do_you_want_to_select_all')}} ( <strong>{{$brands->total()}}</strong> {{trans('applang.items')}} ) ?
                                    </p>
                    ...
                    <div class="col-12 col-md-12 table-responsive">
                        <table class="user-view hover" style="width: 100%; border: 1px solid #ddd">
                            ...
                            @forelse($brands as $index => $brand)
                                <tr class="{{in_array($brand->id ,$checked)? 'table-secondary' : ''}}">
                                    <td class="no-wrap" style="width: 5%">
                                        <fieldset>
                                            <div class="checkbox checkbox-secondary">
                                                <input type="checkbox" id="brand-{{$brand->id}}" value="{{$brand->id}}" wire:model="checked" class="item_checkbox">
                                                <label for="brand-{{$brand->id}}"></label>
                                            </div>
                                        </fieldset>
                                    </td>
                                    <td class="no-wrap" style="width: 5%">{{$index + 1}}</td>
                                    <td class="no-wrap">{{$brand->name}}</td>
                                    <td class="">
                                        @foreach($brand->sections as $section)
                                            <span class="badge badge-secondary" style="margin-bottom: 1px; margin-top: 2px">{{$section->name}}</span>
                                        @endforeach
                                    </td>
                                    <td class="no-wrap"><span class="badge {{$brand->status == 1 ? 'badge-light-success' : 'badge-light-danger'}}">{{$brand->status == 1 ? trans('applang.active') : trans('applang.suspended')}}</span></td>
                                    <td class="no-wrap">
                                        <a href="#" data-toggle="modal" data-target="#formModalEditBrand" wire:click.prevent="editBrand({{$brand->id}})"><i class="bx bx-edit-alt"></i></a>
                                        @if(count($checked) < 2)
                                            <a href="#" class="text-danger mr-1 ml-1" wire:click.prevent="confirmDelete('{{$brand->id}}','{{$brand->name}}')"><i class="bx bx-trash"></i></a>
                                        @endif
                                    </td>
                                </tr>
                            @empty
            ...
            <div class="d-flex d-flex justify-content-between custom-pagination mt-1">
                ...
                <p>
                    ...
                    {{count(\App\Models\ERP\Settings\Products\Brand::all())}}
    ...
    @include('erp.settings.products.brands.modals')
